...
func SetupRoutes(s *Server) {
	...
	if s.config.OAUTH.IsEnabled() || s.config.OIDC.IsEnabled() {
		...
		if s.config.OAUTH.Github.Enabled {
			oauth.POST(fmt.Sprintf("/%s/login", oauthproviders.ProviderGithub), s.OAuthLogin)
		}

		if s.config.OAUTH.Google.Enabled {
			oauth.POST(fmt.Sprintf("/%s/login", oauthproviders.ProviderGoogle), s.OAuthLogin)
		}
	...
	if s.config.OIDC.IsEnabled() {
		oidc := s.server.Group("/oidc")
		oidc.Use(csrfMiddleware)

		for i := range s.config.OIDC {
			oidc.POST(fmt.Sprintf("%s", s.config.OIDC[i].LoginURL), s.OAuthLogin)
		}
	}
